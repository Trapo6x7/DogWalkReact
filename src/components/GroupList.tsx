...
const GroupList: React.FC<GroupListProps> = ({ groups, onShowDetails, onJoinGroup }) => {
  const [showAllModal, setShowAllModal] = useState(false);
  const [allGroups, setAllGroups] = useState<Array<{ id: number; name: string; role?: string }>>([]);
  const { user } = useAuth();
        ...
        <div className="block md:hidden w-full">
          <details className="w-full max-w-md mx-auto" role="region" aria-labelledby="group-list-title">
            <summary className="mb-7 cursor-pointer select-none list-none [&::-webkit-details-marker]:hidden before:hidden">
              <h2 id="group-list-title" className="text-lg font-bold text-secondary-brown uppercase text-center">Mes groupes</h2>
            </summary>
              ...
              {visibleGroups.length > 0 ? (
                visibleGroups.map((group) => (
                  <details key={group.id} className="w-full max-w-md mx-auto" aria-label={`Groupe ${group.name}`}>
                    ...
                    <section className="flex flex-col gap-2 mt-4 px-2 pb-2 uppercase" aria-label={`Détails du groupe ${group.name}`}> 
          ...
          <div className="flex flex-col gap-2 md:gap-3 max-h-35 overflow-y-auto">
            {visibleGroups.length > 0 ? (
              visibleGroups.map((group) => (
                ...
                >
                  <h3 className="text-lg font-bold text-secondary-brown w-1/4 min-w-[120px] text-center uppercase">{group.name}</h3>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-neutral-white/50 backdrop-blur-sm">
            <div className="bg-white p-6 rounded shadow-lg w-full max-w-md mx-4">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={closeAllModal} aria-label="Fermer modal" className="text-xl">×</button>
              </div>
              <div className="py-4">
                <input
                  type="text"
              ...
              <ul className="max-h-60 overflow-y-auto pt-5">
                {allGroups.filter((g) =>
                  ...
                  allGroups
                    ...
                    if (isMember) {
                      ...
                      label = roleKey === 'CREATOR' ? 'Créateur' : roleKey === 'MEMBER' ? 'Membre' : g.role || '';
                    } else {
                      label = requested ? 'Envoyé' : 'Rejoindre';
                    }
                    return (
                      <li key={g.id} className="py-2 flex justify-between items-center hover:bg-gray-100">
                        <span className="cursor-pointer uppercase font-bold text-[var(--primary-brown)]" onClick={() => onShowDetails(g.id)}>
                          {g.name}
                        </span>
                        <button
                          onClick={() => {
